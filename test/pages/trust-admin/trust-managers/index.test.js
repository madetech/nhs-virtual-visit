import { getServerSideProps } from "../../../../pages/trust-admin/trust-managers/index";
...
describe("trust-admin/hospitals", () => {
  ...
  const retrieveTrustManagersByTrustIdSuccessSpy = jest.fn(async () => ({
    trustManagers: [
      { id: 1, email: "1@nhs.co.uk", status:"active" },
      { id: 2, email: "2@nhs.co.uk", status:"disabled" },
    ],
    error: null,
  }));
  ...
  beforeEach(() => {
    ...
    container = {
      ...
      getRetrieveTrustManagersByTrustId: () => retrieveTrustManagersByTrustIdSuccessSpy,
  ...
  describe("getServerSideProps", () => {
    ...
    it("retrieves trust managers", async () => {
      ...
      expect(retrieveTrustManagersByTrustIdSuccessSpy).toHaveBeenCalledWith(
        trustId,
        { withWards: true }
      );
