...
const deleteVisitConfirmation = ({
  ...
  const onSubmit = useCallback(async (event) => {
    ...
    Router.push(
      `/visits/delete-visit-success?wardId=${wardId}&callId=${callId}`
    );
  });
  ...
  return (
    <Layout title="Confirm delete visit">
      <GridRow>
        <GridColumn width="two-thirds">
          <form onSubmit={onSubmit}>
            <Heading>Confirm you want to delete this visit</Heading>
            ...
            <Button className="nhsuk-u-margin-top-5">Confirm delete</Button>
...
export const getServerSideProps = propsWithContainer(
  ...
  async ({ query, container }) => {
    const { wardId, callId } = query;
    console.log(wardId, callId, query, container);
